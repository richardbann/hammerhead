...
class Process:
    ...
    def __str__(self):
        return f"{self.name}: (returncode {'-' if not self.proc else self.proc.returncode})"
        ...
        while others:
            ...
            for p in others:
                if p.proc:
                    ...
                    t._process = p
                    wait_set.add(t)
            completed, others = await asyncio.wait(wait_set, return_when=asyncio.FIRST_COMPLETED, timeout=1)
                    ...
                    if self.terminated:
                        ...
                        if retcode == 0:
                            ...
                            if comp.restart_policy == ON_FAILURE:
                                log.info(f"RESTARTING {comp} (restart_policy = ON_FAILURE)")
                                await comp.start()
                                others.add(comp)
